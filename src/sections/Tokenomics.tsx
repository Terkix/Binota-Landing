  ...
  return (
    <section className="flex text-white justify-center select-none bg-black">
      <div className="container px-8 py-24 md:p-24 mx-0 sm:mx-12 lg:mx-24 max-w-[1440px]">
        ...
        <h4 className={TypeStyles.h4}>Total Supply: {formatCount(data.totalSupply)} BNT</h4>

        {/* Allocation Cards Grid */}
        <div className="items-stretch grid grid-cols-4 xl:grid-cols-8 mt-12">
          {data.allocations.map((allocation) => (
            <div key={allocation.name} className="col-span-4 lg:col-span-4 xl:col-span-4">
              <div className={`${cardStyles.cardGrid} p-12`}>
                <div className="flex items-start gap-3 mb-4">
                  <div className="w-4 h-4 rounded-full mt-1 shrink-0" style={{ backgroundColor: allocation.color }} />
                  <h6 className={`${TypeStyles.h6} mb-0`}>{allocation.name.toUpperCase()}</h6>
                </div>
                <div className="text-3xl font-bold text-primary-200 mb-2">{allocation.percentage}%</div>
